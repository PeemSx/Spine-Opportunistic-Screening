...
def draw_overlay(
    ...
    if refiner_diagnostics:
        for item in refiner_diagnostics[:max_draw]:
            x0, y0, x1, y1 = item["roi_original_xyxy"]
            accepted = bool(item["accepted"])
            color = "deepskyblue" if accepted else "orange"
            rectangle_x = [x0, x1, x1, x0, x0]
            rectangle_y = [y0, y0, y1, y1, y0]
            ax.plot(rectangle_x, rectangle_y, color=color, linewidth=0.7, alpha=0.75)
            ax.text(
                float(x0),
                float(y0),
                f"R{item['candidate_index'] + 1} {'OK' if accepted else item['reason']}",
                color="white",
                fontsize=5,
                ha="left",
                va="bottom",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional corner-refiner checkpoint; rejected refinements retain CenterNet corners.",
    )
    ...
    parser.add_argument(
        "--refiner-allow-boundary-peaks",
        action="store_true",
        help="Accept otherwise valid refinements whose heatmap argmax touches a crop boundary.",
    )
    parser.add_argument(
        "--refiner-no-nearest-anchor",
        action="store_true",
        help="Disable the detector-anchor identity-switch rejection rule.",
    )
    parser.add_argument("--refiner-no-amp", action="store_true")
    parser.add_argument(
        "--show-refiner-rois",
        action="store_true",
        help="Draw detector-generated refiner ROIs and accept/fallback reasons.",
    )
    parser.add_argument(
        "--save-morphology",
        action="store_true",
...
def main() -> None:
    ...
    if refiner_model is not None and refiner_config is not None:
        ...
        print(
            ...
            f"identity offset limit: {refiner_config.max_center_offset_fraction}"
        )
    ...
    for image_index, image_path in enumerate(images, start=1):
            ...
            refiner_json_output.append(
                {
                    ...
                    "accepted_count": sum(item["accepted"] for item in refiner_diagnostics),
        ...
        draw_overlay(
            ...
            refiner_diagnostics=refiner_diagnostics if args.show_refiner_rois else None,
        )
        if args.spine_chain:
            refiner_text = ""
            if refiner_diagnostics:
                accepted_count = sum(item["accepted"] for item in refiner_diagnostics)
                refiner_text = f" | refiner {accepted_count}/{len(refiner_diagnostics)} accepted"
            print(
